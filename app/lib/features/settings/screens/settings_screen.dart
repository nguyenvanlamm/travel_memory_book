...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ThemeMode _themeMode = ThemeMode.system; bool _notificationsEnabled = false; bool _pageFlipSoundEnabled = true;
  @override void initState() { super.initState(); _loadSettings(); }
  Future<void> _loadSettings() async { final prefs = await SharedPreferences.getInstance(); final audioService = ref.read(pageFlipAudioServiceProvider); await audioService.init(); if (mounted) setState(() { _themeMode = ThemeMode.values[prefs.getInt('theme_mode') ?? 0]; _notificationsEnabled = prefs.getBool('notifications') ?? false; _pageFlipSoundEnabled = audioService.enabled; }); }
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Settings')), body: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      const SizedBox(height: 24), SectionHeader(title: 'Reading Experience'),
